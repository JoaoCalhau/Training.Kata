namespace Training.Kata.Weight_for_weight
{
    using System.Linq;
    ...
    public class Kata
    {
        public static string orderWeight(string strng)
        {
            return string.Join(' ', strng.Split(' ')
                .OrderBy(item => Enumerable.Sum(item.ToCharArray().Select(number => int.Parse(number.ToString()))))
                .ThenBy(item => item));
        }

        public static string orderWeight_MostRatedSolution(string strng)
        {
            return string.Join(" ", strng.Split(' ')
